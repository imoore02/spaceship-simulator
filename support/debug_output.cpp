...
// Debug callback
#if !defined(NDEBUG)
void GLAPIENTRY callback_gldebug_(GLenum, GLenum, GLuint, GLenum, GLsizei,
...
void GLAPIENTRY callback_gldebug_(GLenum, GLenum aType, GLuint,
                                  GLenum aSeverity, GLsizei,
  ...
  if (GL_DEBUG_TYPE_OTHER == aType) return;

  std::fprintf(stderr, "OpenGL Debug: %s [%s]: %s\n", severity_str_(aSeverity),
               type_str_(aType), aMessage);

  // For high severity errors, break into the debugger now.
  if (GL_DEBUG_SEVERITY_HIGH == aSeverity) assert(false);
}
#endif  // ~ !NDEBUG
}  // namespace
